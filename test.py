...
interval = "5m"
base_timesteps = 20
lookback_minutes = 100  # Giảm xuống 100 phút thay vì 600
...
risk_per_trade = 0.02
confidence_threshold = 0.15  # Giảm xuống 0.15
adx_threshold = 5  # Giảm xuống 5
rsi_overbought = 80  # Nới lên 80
rsi_oversold = 20  # Nới xuống 20
stop_loss_target = 0.05  # Giảm xuống 0.05
take_profit_percent = 0.08  # Tăng lên 0.08
base_confidence_threshold = 0.20  # Giảm xuống 0.20
...
# Lưu trữ dữ liệu scaled cho tối ưu
    ...
    while retries < max_retries:
        try:
            ...
            if ohlcv and len(ohlcv) > 0:
                ...
                updated_df = pd.concat([historical_df.iloc[-100:], new_data]).drop_duplicates()  # Giới hạn 100 nến
...
def add_features_incremental(df, prev_df=None):
    ...
    # Tái sử dụng giá trị trước đó cho các tính toán
    if prev_df is not None:
        new_df = pd.concat([prev_df.iloc[-34:], new_df]).drop_duplicates()  # Giới hạn 34 nến cho tính toán
    min_length = 34
    ...
    new_df["rsi"] = RSIIndicator(close=new_df["close"], window=14).rsi().shift(1).fillna(method='bfill', limit=1)
    new_df["atr"] = AverageTrueRange(high=new_df["high"], low=new_df["low"], close=new_df["close"], window=14).average_true_range().shift(1).fillna(method='bfill', limit=1)
    adx = ADXIndicator(high=new_df["high"], low=new_df["low"], close=new_df["close"], window=14)
    new_df["adx"] = adx.adx().shift(1).fillna(method='bfill', limit=1)
    new_df["adx_pos"] = adx.adx_pos().shift(1).fillna(method='bfill', limit=1)
    new_df["adx_neg"] = adx.adx_neg().shift(1).fillna(method='bfill', limit=1)
    new_df["adx_signal"] = np.where(new_df["adx_pos"] > new_df["adx_neg"], 1, -1)
    new_df["stoch"] = StochasticOscillator(high=new_df["high"], low=new_df["low"], close=new_df["close"], window=14, smooth_window=3).stoch().shift(1).fillna(method='bfill', limit=1)
    new_df["momentum"] = new_df["close"].diff(5).shift(1).fillna(method='bfill', limit=1)
    new_df["momentum"] = np.log1p(new_df["momentum"].abs()) * np.sign(new_df["momentum"])
    new_df["awesome"] = awesome_oscillator(high=new_df["high"], low=new_df["low"], window1=5, window2=34).shift(1).fillna(method='bfill', limit=1)
...
def generate_signal(df):
    ...
    adjusted_threshold = base_confidence_threshold - 0.1 if latest_adx < 20 else base_confidence_threshold  # Giảm thêm 0.1
    ...
    min_price_change_threshold = 0.00005  # Giảm xuống 0.005%
...
def calculate_trailing_stop(entry_price, current_price, latest_atr, signal):
    ...
    trailing_stop_factor = 0.6 * atr_ratio  # Tăng lên 0.6
    if signal == "LONG":
        trailing_stop_price = current_price * (1 - trailing_stop_factor) if current_price > entry_price + 0.5 * latest_atr else trailing_stop_price
    else:
        trailing_stop_price = current_price * (1 + trailing_stop_factor) if current_price < entry_price - 0.5 * latest_atr else trailing_stop_price
...
async def main():
    global current_position, total_trades, winning_trades, capital, daily_trades, daily_profit, is_trading_paused, last_drawdown_check, total_loss, last_candle_time, data_scaled_buffer
    ...
    while True:
        if time.time() - last_drawdown_check >= 3600:  # Kiểm tra drawdown mỗi giờ
            drawdown = calculate_drawdown()
        ...
        if current_position is not None:
            ...
            logging.info(f"Current position: {signal}, Entry: {entry_price:.2f}, Current: {current_price:.2f}, PnL: {profit_percent:.2f}% ({profit_usdt:.2f} USDT), Leverage: {current_leverage}x")
            ...
            take_profit_percent = take_profit_percent / current_leverage
        ...
        historical_df = df
        if time.time() % 86400 < 10:  # Lưu mỗi ngày thay vì mỗi giờ
            save_trade_history()
            save_capital_history()
            save_market_data()
            send_status_update()
        ...
        await asyncio.sleep(0.5)  # Giảm xuống 0.5 giây
